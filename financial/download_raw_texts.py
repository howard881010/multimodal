...
from src.utils import download_raw_texts_from_urls, save_text_to, get_logger
import logging

if __name__ == "__main__":
    data_dir = "/data/kai/forecasting/summary"
    paths = glob("/data/kai/forecasting/raw_urls/*.csv")

    for directory_path in tqdm(paths, total=len(paths)):
        df = pd.read_csv(directory_path)
        ticker = df.iloc[0]["ticker"]

        get_logger(f"logs/{ticker}_raw.txt")
